...
function Estadisticas({ rol }) {  // Declaración del componente Estadisticas con el argumento 'rol'
    ...
    useEffect(() => {
        ...
        const stock = productos.map((producto) => producto.stock);  // Extrae las cantidades de los productos
        ...
        const almacen = new Chart(ctx, {  // Crea un nuevo gráfico de tipo 'bar' con Chart.js y lo asigna al elemento canvas
            ...
            datasets: [{
                ...
                data: stock,  // Asigna las cantidades de productos para la visualización
    ...
    const generarReporteAlmacen = () => {
        ...
        .then((productos) => {
            ...
            doc.save("Reporte de almacén.pdf");  // Descarga el documento PDF con el nombre 'reporte_almacen.pdf'
        })
        .catch((error) => console.error('Error al obtener los productos:', error));  // Manejo de errores en caso de fallar la solicitud
    };
    ...
    try {
        ...
        pdf.text("Reporte de almacén", 20, 10);
        ...
        pdf.save("Reporte gráfico de almacén.pdf");
    } catch (error) {
